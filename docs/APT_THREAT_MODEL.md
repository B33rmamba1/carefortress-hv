...
**virtio-serial channel authentication (added June 2026):** Each guest agent entry is HMAC-SHA256 signed using a pre-shared key and carries a monotonic sequence number. The host collector verifies the signature and detects sequence gaps before accepting entries. An attacker who kills the guest agent and hijacks the virtio port will have their injected entries flagged as AGENT_SECURITY_ALERT events in the chain with NO_HMAC or HMAC_FAIL warnings. Limitation: the HMAC key is stored on the guest at /etc/carefortress-agent.key (root-only, 600). A root-privileged attacker who specifically extracts the key before injecting can forge valid signatures. Sequence gap detection (SEQ_REWIND) partially mitigates this by detecting agent restarts. Full mitigation requires TPM-sealed keys (future work).
