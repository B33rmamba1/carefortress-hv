...
**virtio-serial channel authentication (added June 2026):** Each guest agent entry is HMAC-SHA256 signed using a pre-shared key and carries a monotonic sequence number. The host collector verifies the signature and detects sequence gaps before accepting entries. An attacker who kills the guest agent and hijacks the virtio port will have their injected entries flagged as AGENT_SECURITY_ALERT events in the chain with NO_HMAC or HMAC_FAIL warnings.

**Known limitation:** The HMAC key is provisioned to the guest at deployment time. A root-privileged attacker who compromises the guest can read the key and forge valid signatures. Sequence gap detection (SEQ_REWIND) partially mitigates this by detecting agent restarts and sequence discontinuities. Full mitigation requires TPM-sealed key provisioning so the key can only be unsealed in a measured boot state -- this is planned as a future release item.
